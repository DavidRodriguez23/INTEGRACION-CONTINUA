...
function json_decode_rec($json, $depth = 3) {
    $d = $json;
    while (is_string($d) && $depth-- > 0) $d = json_decode($d, true);
    return $d;
}

$imagenes_raw = json_decode_rec($animal->obtener_imagenes()) ?? [];
$imagenes = [];
...
$es_premium = method_exists($animal, 'es_premium') ? $animal->es_premium() : false;

$carrito = $_SESSION['carrito'] ?? [];
$en_carrito = false;
foreach ($carrito as $item) {
    if (isset($item['id'], $item['tipo']) && $item['id'] == $animal->obtener_id() && $item['tipo'] === 'animal') {
        $en_carrito = true; break;
    }
}

$titulo_safe    = htmlspecialchars($animal->obtener_titulo(), ENT_QUOTES, 'UTF-8');
$depto_safe     = htmlspecialchars($animal->obtener_departamento(), ENT_QUOTES, 'UTF-8');
$municipio_safe = htmlspecialchars($animal->obtener_municipio(), ENT_QUOTES, 'UTF-8');
?>
...
<div class="container" style="padding:40px 0 60px;">
  ...
  <nav style="margin-bottom:24px;">
    <span style="font-size:0.85rem;color:var(--tierra-mid);">
      ...
      <span style="margin:0 8px;">›</span>
      <span style="color:var(--tierra);"><?= $titulo_safe ?></span>
    </span>
  </nav>
    ...
    <div class="col-lg-7">
      ...
      <div style="background:var(--crema-mid);border-radius:var(--radio-lg);overflow:hidden;aspect-ratio:4/3;display:flex;align-items:center;justify-content:center;border:1px solid rgba(61,43,31,0.1);">
        <?php if (!empty($imagenes)): ?>
          <img src="<?= htmlspecialchars($imagenes[0], ENT_QUOTES, 'UTF-8') ?>"
               alt="<?= $titulo_safe ?>"
        ...
        <div style="display:flex;gap:8px;margin-top:10px;overflow-x:auto;padding-bottom:4px;">
          <?php foreach (array_slice($imagenes, 1) as $img): ?>
            <img src="<?= htmlspecialchars($img, ENT_QUOTES, 'UTF-8') ?>"
                 style="width:80px;height:60px;object-fit:cover;border-radius:6px;border:2px solid var(--crema-dark);cursor:pointer;flex-shrink:0;">
          <?php endforeach; ?>
        </div>
      <?php endif; ?>
        ...
        <div style="margin-top:24px;">
          ...
          <script>
            document.addEventListener('DOMContentLoaded', function() {
              var mapa = L.map('mapa-animal').setView([<?= (float)$latitud ?>, <?= (float)$longitud ?>], 12);
              L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {attribution:'&copy; OpenStreetMap'}).addTo(mapa);
              L.marker([<?= (float)$latitud ?>, <?= (float)$longitud ?>]).addTo(mapa)
                .bindPopup('<strong><?= addslashes($titulo_safe) ?></strong><br><?= addslashes($municipio_safe . ', ' . $depto_safe) ?>').openPopup();
            });
          </script>
        </div>
      <?php endif; ?>
      ...
      <div style="position:sticky;top:84px;">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px;">
          <span style="background:var(--dorado-pale);color:var(--dorado);font-size:0.72rem;font-weight:500;padding:4px 12px;border-radius:20px;letter-spacing:0.05em;text-transform:uppercase;">
            <?= htmlspecialchars($animal->obtener_categoria(), ENT_QUOTES, 'UTF-8') ?>
          </span>
          <span style="background:rgba(45,80,22,0.1);color:var(--verde);font-size:0.72rem;font-weight:500;padding:4px 12px;border-radius:20px;letter-spacing:0.05em;text-transform:uppercase;">
            <?= htmlspecialchars($animal->obtener_raza(), ENT_QUOTES, 'UTF-8') ?>
          </span>
          <?php if ($es_premium): ?>
        ...
        <div style="background:var(--crema-mid);border-radius:var(--radio-lg);padding:16px 20px;margin-bottom:20px;border-left:4px solid var(--dorado);">
          ...
          <?php if ($animal->obtener_tipo_precio()): ?>
            <div style="font-size:0.8rem;color:var(--tierra-mid);">por <?= htmlspecialchars($animal->obtener_tipo_precio(), ENT_QUOTES, 'UTF-8') ?></div>
          <?php endif; ?>
        </div>
        ...
        <div style="background:#fff;border:1px solid rgba(61,43,31,0.1);border-radius:var(--radio-lg);padding:16px 20px;margin-bottom:20px;">
          ...
          <table style="width:100%;font-size:0.88rem;">
            <?php
            $pureza = method_exists($animal,'obtener_pureza') ? $animal->obtener_pureza() : null;
            $detalles = [
            ...
            ?>
              <tr style="border-bottom:1px solid rgba(61,43,31,0.06);">
                <td style="padding:7px 0;color:var(--tierra-mid);width:42%;">
                  <i class="<?= $d[0] ?> me-2" style="color:var(--dorado);width:16px;text-align:center;"></i>
                  <?= $d[1] ?>
                </td>
                <td style="padding:7px 0;color:var(--tierra);font-weight:500;">
                  <?= htmlspecialchars($d[2], ENT_QUOTES, 'UTF-8') ?>
                </td>
              </tr>
            <?php endforeach; ?>
          </table>
        ...
        <?php if ($animal->obtener_descripcion()): ?>
          <div style="background:#fff;border:1px solid rgba(61,43,31,0.1);border-radius:var(--radio-lg);padding:16px 20px;margin-bottom:20px;">
            ...
            <p style="font-size:0.9rem;color:var(--tierra-mid);line-height:1.7;margin:0;">
              <?= nl2br(htmlspecialchars($animal->obtener_descripcion(), ENT_QUOTES, 'UTF-8')) ?>
            </p>
          </div>
        <?php endif; ?>
        ...
        <div style="background:var(--crema-mid);border-radius:var(--radio-lg);padding:16px 20px;margin-bottom:20px;">
          ...
          <?php if ($animal->obtener_telefono()): ?>
            <a href="https://wa.me/57<?= preg_replace('/[^0-9]/','',$animal->obtener_telefono()) ?>?text=Hola%2C+me+interesa%3A+<?= urlencode($animal->obtener_titulo()) ?>"
               target="_blank"
          ...
          <?php if ($animal->obtener_correo()): ?>
            <a href="mailto:<?= htmlspecialchars($animal->obtener_correo(), ENT_QUOTES, 'UTF-8') ?>?subject=Consulta+sobre%3A+<?= urlencode($animal->obtener_titulo()) ?>"
        ...
        <?php if (!$animal->esta_vendido()): ?>
          <form method="post" action="<?= SERVIDOR ?>/vistas/carrito.php">
            <input type="hidden" name="id" value="<?= (int)$animal->obtener_id() ?>">
            <input type="hidden" name="tipo" value="animal">
            <input type="hidden" name="accion" value="<?= $en_carrito ? 'eliminar' : 'agregar' ?>">
            <button type="submit" class="btn btn-lg w-100"
              style="background:<?= $en_carrito ? 'var(--tierra-mid)' : 'var(--dorado)' ?>;color:<?= $en_carrito ? '#fff' : 'var(--tierra)' ?>;border:none;font-weight:600;padding:14px;">
